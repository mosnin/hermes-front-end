  ...
  return (
    <header className="absolute inset-x-0 top-0 z-40">
      <div className="mx-auto flex h-16 max-w-[1060px] items-center justify-between px-5 sm:px-7">
        <Link href="/" className="flex items-center gap-1.5 text-[15px] font-semibold tracking-[0.12em] text-[var(--site-ink)]">
          <Mark />
          CADRE
        </Link>
  ...
  return (
    <footer className="relative mt-24">
      <div className="absolute inset-0">
        <Painting scene="footer artwork" className="h-full w-full" />
      </div>
      <div className="relative mx-auto max-w-[1060px] px-5 pb-10 pt-16 sm:px-7">
        <div className="grid grid-cols-2 gap-x-6 gap-y-10 sm:grid-cols-4">
          {FOOTER_COLS.map((col) => (
            <div key={col.title}>
              <p className="mb-4 text-[15px] font-semibold text-[#3c3a35]">{col.title}</p>
              <ul className="space-y-2.5 text-[14.5px] text-[#75726c]">
                {col.links.map((l) => (
                  <li key={l.label}>
                    <Link href={l.href} className="transition hover:text-[#1f1f1c]">
                      {l.label}
                    </Link>
                  </li>
                ))}
        ...
        <div className="mt-56 flex flex-wrap items-center gap-x-6 gap-y-2 text-[13px] text-[#75726c]">
          <span>© Cadre {new Date().getFullYear()}</span>
          <Link href="/about" className="hover:text-[#1f1f1c]">Privacy Policy</Link>
          <Link href="/status" className="hover:text-[#1f1f1c]">Security</Link>
        </div>
      </div>
      <div className="relative bg-[#101210]">
        <div className="mx-auto flex max-w-[1060px] items-center justify-between px-5 py-5 sm:px-7">
